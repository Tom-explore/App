...
class TripAttributeController {
  static async createAttribute(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res.status(201).json({
        message: 'Trip attribute created successfully',
        tripAttribute,
      });
    } catch (error: any) {
      return res.status(400).json({
        message: 'Error creating trip attribute',
        error: error.message,
      });
    }
  }
  ...
  static async getAttributeById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const tripAttribute = await TripAttribute.findAttributeById(
        Number(tripId),
        Number(attributeId)
      );
      if (!tripAttribute) {
        return res.status(404).json({ message: 'Trip attribute not found' });
      }
      return res.status(200).json(tripAttribute);
    } catch (error: any) {
      return res.status(400).json({
        message: 'Error fetching trip attribute',
        error: error.message,
      });
    }
  }
  ...
  static async getAttributesByTrip(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res.status(400).json({
        message: 'Error fetching trip attributes by trip',
        error: error.message,
      });
    }
  }
  ...
  static async updateAttribute(req: Request, res: Response): Promise<Response> {
    try {
      const { tripId, attributeId } = req.params;
      const data = req.body;
      const updatedAttribute = await TripAttribute.updateAttribute(
        Number(tripId),
        Number(attributeId),
        data
      );
      ...
      return res.status(200).json({
        message: 'Trip attribute updated successfully',
        updatedAttribute,
      });
    } catch (error: any) {
      return res.status(400).json({
        message: 'Error updating trip attribute',
        error: error.message,
      });
    }
  }
  ...
  static async deleteAttribute(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res.status(400).json({
        message: 'Error deleting trip attribute',
        error: error.message,
      });
    }
  }
