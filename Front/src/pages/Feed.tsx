...
const Feed: React.FC = () => {
  ...
  const horizontalGap = useMemo(
    () => containerWidth * (HORIZONTAL_GAP_PERCENT / 100),
    [containerWidth]
  );
  const verticalGap = useMemo(
    () => containerWidth * (VERTICAL_GAP_PERCENT / 100),
    [containerWidth]
  );

  const ITEM_WIDTH = useMemo(
    () => (containerWidth - (COLUMN_COUNT - 1) * horizontalGap) / COLUMN_COUNT,
    [containerWidth, COLUMN_COUNT, horizontalGap]
  );
  const rowHeight = useMemo(
    () => ITEM_HEIGHT + verticalGap,
    [ITEM_HEIGHT, verticalGap]
  );
  const rowCount = useMemo(
    () => Math.ceil(sortedFilteredPlaces.length / COLUMN_COUNT),
  ...
  const memoizedHandleCategoryChange = useCallback(
    (categoryId: number) => {
  ...
  const memoizedHandleAttributeChange = useCallback(
    (attributeId: number) => {
  ...
  const memoizedGetTranslation = useCallback(
    (slug: string, type: "attributes" | "categories") => {
  ...
  const Cell = useCallback(
    ({
      ...
      const distance = useMemo(
        () =>
          geolocation
            ? calculateDistanceFromPlace(geolocation, {
                lat: place.lat,
                lng: place.lng,
              })
            : undefined,
      ...
      return (
        <div
          style={{
            ...style,
            paddingRight:
              columnIndex < COLUMN_COUNT - 1 ? `${horizontalGap}px` : "0px",
        ...
        <IonContent className="ion-no-padding">
          <IonGrid>
            <IonRow>
              ...
              <IonCol size="12" sizeXl="3" sizeMd="4" className="ion-hide-md-down">
                  <FilterPlaces
                    categories={uniqueCategories}
                    attributes={uniqueAttributes}
                    selectedCategories={selectedCategories}
